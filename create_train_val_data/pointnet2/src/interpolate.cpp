...
//    cudaStream_t stream = THCState_getCurrentStream(state);
    {
        at::cuda::CUDAGuard device_guard{unknown_tensor.device().index()};
        cudaStream_t stream = at::cuda::getCurrentCUDAStream(idx_tensor.device().index());
...
void three_interpolate_wrapper_fast(int b, int c, int m, int n,
    ...
    {
        at::cuda::CUDAGuard device_guard{points_tensor.device().index()};
        cudaStream_t stream = at::cuda::getCurrentCUDAStream(idx_tensor.device().index());
...
void three_interpolate_grad_wrapper_fast(int b, int c, int n, int m,
    ...
    {
        at::cuda::CUDAGuard device_guard{idx_tensor.device().index()};
        cudaStream_t stream = at::cuda::getCurrentCUDAStream(idx_tensor.device().index());
