...
class self_attention(nn.Module):
    ...
    def forward(self, src1, pos=None):
        src1 = self.input_proj(src1)  # [B, 256, 6]
        b, c, _ = src1.shape
        src1 = src1.reshape(b, c, -1).permute(2, 0, 1)  # [6, B, 256]
        src1 = self.norm13(src1)
        ...
        src12 = self.multihead_attn(query=q,
                                     ...
                                     value=src1)[0]
        src1 = src1 + self.dropout12(src12)
        src1 = self.norm12(src1)  # [6, B, 256]
        src12 = self.linear12(self.dropout1(self.activation1(self.linear11(src1))))
        src1 = src1 + self.dropout13(src12)
        src1 = src1.permute(1, 2, 0)  # [B, 256, 6]
...
class PCViews:
    ...
    def __init__(self,TRANS,RESOLUTION):
        _views = np.asarray([
            ...
            [[np.pi / 6, -np.pi / 6, np.pi / 3], [0, 0, TRANS]],
            [[np.pi / 6, -np.pi / 6, 5*np.pi / 6], [0, 0, TRANS]],
            [[np.pi, np.pi / 6, np.pi / 2], [0, 0, TRANS]],
            [[np.pi, np.pi / 6, 2*np.pi / 3], [0, 0, TRANS]],
            [[np.pi / 6, -np.pi / 3, np.pi / 3], [0, 0, TRANS]],
            [[np.pi / 6, -np.pi / 3, 2*np.pi / 3], [0, 0, TRANS]],
            [[np.pi, np.pi / 3, np.pi / 3], [0, 0, TRANS]],
            [[np.pi, np.pi / 3, 2*np.pi / 3], [0, 0, TRANS]],
            [[np.pi / 2, np.pi / 6, np.pi / 3], [0, 0, TRANS]],
            [[np.pi / 2, np.pi / 6, 2*np.pi / 3], [0, 0, TRANS]],
            [[-np.pi / 2, np.pi / 6, np.pi / 3], [0, 0, TRANS]],
            [[-np.pi / 2, np.pi / 6, 2*np.pi / 3], [0, 0, TRANS]]])
        self.num_views = 3
        # self.num_views = 6
        ...
        # select_nums = [3, 9, 13]
        select_nums = [0, 1, 2]
        # select_nums = [4, 5, 7, 10, 13, 14]
        select_views = _views[select_nums, :, :]
        angle = torch.tensor(select_views[:, 0, :]).float().cuda()
        # angle = torch.tensor(_views[:, 0, :]).float().cuda()
        self.rot_mat = euler2mat(angle).transpose(1, 2)
        self.translation = torch.tensor(select_views[:, 1, :]).float().cuda()
